...
fn imports_into_an_empty_collection_and_preserves_note_fields() {
    ...
    let backup_json = export_flat_collection_backup(std::slice::from_ref(&original_note)).unwrap();
...
fn merge_import_replaces_same_identity_notes_without_creating_duplicates() {
    ...
    let backup_json =
        export_flat_collection_backup(&[replacement_note.clone(), imported_only.clone()]).unwrap();
