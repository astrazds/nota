...
- Keep native persistence concrete. Store a versioned `collection.json` under the XDG application data directory, preserve the last valid state as `collection.previous.json`, and keep Theme/window preferences and Backup Health in separate files.
